...
const CommentDialog = ({open,setOpen}) => {
    ...
    const addCommentHandler = async()=>{
        try {
            const res = await axios.post(`http://localhost:8000/api/v1/post/${selectedPost?._id}/comment`,{text:comment}, {
                headers: {
            ...
            if(res.data.success){
                const updatedPosts = posts.map((post)=> post._id === selectedPost._id ? { ...post,comments: [...post.comments, res.data.comment] } : post)
                dispatch(setSelectedPost({...selectedPost, comments: [...selectedPost.comments, res.data.comment]}))
